...
\item Incluye Grid system, útil para diseñar y maquetar por columnas. Este sistema permite añadir hasta un total de doce columnas en la página.
...
En cada uno de los siete aspectos se podía obtener un total de 3 puntos, lo que conllevaba una puntuación máxima de 21 puntos. Dependiendo de la puntuación obtenida, el usuario podía tener un nivel \textit{basic}, \textit{developing} o \textit{proficiency}, tal y como se muestra en la \textit{Tabla \ref{tabla:clasif1}}
...
\item La puntuación máxima se consigue usando bloques de nivel 3 en todas las categorías y obteniendo todos los \emph{bonus}.
...
Finalmente, para motivar aún más a los niños, se decidió añadir \emph{bonus}. Por el momento hay 4 tipos de \emph{bonus} disponibles:

\begin{enumerate}

\item \textbf{Bonus por diversidad}: se obtiene cuando el usuario utiliza bloques pertenecientes a los tres niveles de una misma categoría.
...
\item \textbf{Bonus por todas las categorías}: Como ya sabemos, Pencil Code está formado por seis categorías principales, este \emph{bonus} se obtiene si el programa analizado contiene bloques de las seis categorías.
...
\item \textbf{Cabecera superior}: Se habilitó de nuevo el botón \textit{Help} añadiendo tres pasos adicionales al Tour, uno por cada nueva sección: \textit{Details: Used Blocks, Download Certificate y Help us!}. El aspecto del tour es el que se muestra en  \textit{Figura ~\ref{fig:Tourdashboard}}.
...
\item \textbf{Tabla de puntuaciones}: Con la introducción de los \emph{bonus}, se añadió una nueva fila a la tabla de puntuaciones en la que estos quedaban representados con estrellas; se rellenan tantas estrellas como \emph{bonus} se consigan.
...
\item \textbf{What can you improve?}: se introdujeron mensajes donde se aconsejaba al usuario en cual de las categorías debía trabajar más para subir su nivel, así como sugerencias para completar todos los \emph{bonus}. Estos mensajes se centran en las categorías en las que se ha conseguido el nivel más bajo.

\item \textbf{Details: Used Blocks}: para conocer los \emph{bonus} conseguidos de una forma más específica, se añadió a la sección \textit{Details} un desplegable \textit{Bonus} donde se explicaba porqué motivo se habían obtenido. También se añadieron elementos visuales.
...
Para analizar un proyecto con Coffee-mastery se debe introducir el comando \texttt{python coffee-mastery.py colors.coffee}, especificando como parámetro el nombre del archivo CoffeeScript. La salida del programa es la que se muestra en la \textit{Figura \ref{fig:salidacoffee}}.

\begin{figure}[H]
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/salidacoffemastery.png}
  \caption{Salida de coffee-mastery.py}
  \label{fig:salidacoffee}
\end{figure}
...
A la hora de introducir la URL en Dr.Scratch, si ésta no es correcta, pueden aparecer tres tipos de errores: si la URL está mal escrita, si la URL/proyecto no existe en los servidores de Scratch o si en la URL no se ha introducido ningún valor.
...
\item \textit{ProcMastery()} se encarga de montar el diccionario y guardar todo en la base de datos; suma los puntos, comprueba los \emph{bonus} llamando a \textit{CalculoBonus()} y comprueba los mensajes de ayuda de la sección \textit{What can you improve?} llamando a \textit{wtimprove()}. 

\end{enumerate}

En la \textit{Figura \ref{fig:analisis}} del \textit{Apéndice \ref{app:diagrama}} se puede observar el diagrama de flujo del análisis de los proyectos por URL.
...
Las tres páginas restantes (\texttt{Points.html}, \texttt{Bonus.html} y \texttt{Blocks.html}) están destinadas a mostrar los bloques existentes por categoría y a la explicación de cómo Dr.Pencilcode evalúa o analiza los proyectos, es decir, determina las puntuaciones y los \emph{bonus} que se asignan a cada proyecto. \newline
...
El diseño de la página del formulario es el mostrado en la \textit{Figura~\ref{fig:PaginaForm}}

\begin{figure}[H]
  \centering
  \includegraphics[width=14cm, keepaspectratio]{img/formpage.png}
  \caption{Página de formulario}
  \label{fig:PaginaForm}
\end{figure}
...
Para poder comprender el trabajo de la función \textbf{getFeedback}, se muestra su algoritmo en la \textit{Figura \ref{fig:PagForm}} del \textit{Apéndice \ref{app:diagrama}}.
...
A estas modificaciones se incluyen también las nuevas funcionalidades y plugins desarrollados en este proyecto, como las nueve páginas de ayuda (\textit{Move.html}, \textit{Art.html}, \textit{Text.html}, \textit{Sound.html}, \textit{Control.html}, \textit{Operators.html}, \textit{Points.html}, \textit{Bonus.html} y \textit{Blocks.html}), las nuevas secciones en la página de \textit{dashboard.html} (bonus, botones, details, etc), el formulario y el plugin \textit{coffee-mastery.py}. \newline 
...
La sección \textit{What can you improve?} da consejos sobre cómo mejorar tu programa; en este caso, hace hincapié en trabajar las categorías de Operators, Text y Sound y anima al usuario a conseguir todos los \emph{bonus}.\newline

Si el usuario no sabe cómo mejorar su código, éste puede pinchar en los enlaces de las categorías que conducen a las páginas de ayuda; en ellas, se encuentran ejemplos de varios programas donde se explica cómo utilizar los bloques, así como diversas tablas donde se indican los niveles a los que pertenece cada bloque. Por ejemplo, en la página de ayuda de la categoría \textbf{Control} se incluye una guía de cómo llegar al nivel 3 mediante el uso de funciones. Véase la \textit{Figura \ref{fig:level3control}}.
...
Una vez que se han consultado las páginas de ayuda, el usuario puede regresar al código de su proyecto para seguir mejorándolo. Para mejorar el programa Ejemplo 1, se han añadido los bloques de: función -$>$ (nivel 3 de Operators), click (nivel 3 de Control), jumpto (nivel 3 de Move)... Ahora, la tortuga dibuja flores de distintos colores - aleatorios - en las coordenadas donde el usuario hace click con el ratón, \textit{Figura \ref{fig:ejemplo2}}.
...
Como se puede observar, la puntuación del programa ha pasado de 4 a 13. Además de mejorar las puntuaciones específicas de tres categorías, se han conseguido \emph{bonus} por diversidad, complejidad y repetición de bloque; aun así, el usuario podría seguir mejorando el código añadiendo bloques de Text y Sound como se indica en la sección \textit{What can you improve?}. Así mismo, el alumno podría descargar el certificado en el que se indica el nombre del proyecto analizado junto con la puntuación obtenida, \textit{Figura \ref{fig:certifejemplo2}}.\newline
...
\item \textbf{Añadir nuevas funcionalidades y secciones a la página web}. En este objetivo no se especificó un número concreto de adiciones. Las novedades que se llevaron a cabo para alcanzar dicho objetivo fueron: la implementación de un formulario, el diseño de nueve páginas de ayuda, la inserción de \emph{bonus} y el cambio de apariencia del dashboard incluyendo tres nuevas secciones.  
...
Dr. Pencilcode puede encontrarse en la web con los siguientes enlaces:
\begin{itemize}
\item \url{http://drpencilcode.libresoft.info/}

\item \url{http://193.147.79.206/}
...
Para entender las funciones de análisis por URL y del formulario de una forma más sencilla, en este apéndice se incluyen las representaciones gráficas de los respectivos algoritmos.
...
\chapter{Formulario Dr. Pencilcode}
\label{app:formulario}

Antes de realizar la implementación del formulario de Dr. Pencilcode en la página web, existió el formulario a papel que se muestra en este anexo.

\includepdf[pages=1-2, width=\textwidth]{docs/Survey.pdf}
\label{pdf:formulario}


\chapter{Talleres}
\label{app:Talleres}

Como ya se ha mencionado, durante el desarrollo del proyecto tuve la oportunidad de acudir a diversos talleres de Dr. Scratch. Dos de los talleres se realizaron en la Fundación síndrome de Down de Madrid (Down Madrid) donde se introdujo la herramienta a los alumnos y se realizaron diversos programas para, posteriormente, ser analizados. 
El otro taller tuvo lugar en el Foro Internacional de Contenidos Digitales (FICOD 2015) donde se llevo a cabo el taller de superprogramadores de Orange, \textit{Figura \ref{figure:FICOD}}.\newline
